...
`archivebox-api` exposes its MCP server (console script `archivebox-mcp`) four ways. Pick the row that
...
```json
{
  "mcpServers": {
    "archivebox-mcp": {
      "command": "uvx",
      "args": ["--from", "archivebox-api", "archivebox-mcp"],
      "env": {
        "ARCHIVEBOX_BASE_URL": "<your-archivebox_base_url>",
        "ARCHIVEBOX_URL": "<your-archivebox_url>",
        "ARCHIVEBOX_USERNAME": "<your-archivebox_username>"
      }
    }
  }
}
...
```bash
uvx --from archivebox-api archivebox-mcp --transport streamable-http --host 0.0.0.0 --port 8000
...
```json
{
  "mcpServers": {
    "archivebox-mcp": {
      "command": "uvx",
      "args": ["--from", "archivebox-api", "archivebox-mcp", "--transport", "streamable-http", "--port", "8000"],
      "env": {
...
{
  "mcpServers": {
    "archivebox-mcp": { "url": "http://localhost:8000/mcp" }
  }
}
```
  ...
  "mcpServers": {
    "archivebox-mcp": {
      ...
      "args": [
        "run", "-i", "--rm",
        "-e", "TRANSPORT=stdio",
        "-e", "ARCHIVEBOX_BASE_URL=<your-archivebox_base_url>",
        "-e", "ARCHIVEBOX_URL=<your-archivebox_url>",
        "-e", "ARCHIVEBOX_USERNAME=<your-archivebox_username>",
...
```bash
docker run -d --name archivebox-mcp -p 8000:8000 \
...
```bash
uv run archivebox-mcp --transport streamable-http --port 8000
```
...
{
  "mcpServers": {
    "archivebox-mcp": { "url": "http://archivebox-mcp.arpa/mcp" }
  }
}
```

Caddy reverse-proxies `http://archivebox-mcp.arpa` to the container's `:8000`
streamable-http listener; `http://archivebox-mcp.arpa/health` returns
